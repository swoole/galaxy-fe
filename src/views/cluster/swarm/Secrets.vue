<template>
  <div class="project-container swarm-secrets" v-loading="loading">
    <breadcrumb :breadcrumb="breadcrumb" :cluster="cluster" />

    <el-card shadow="never" class="content-card" style="margin: 20px">
      <div slot="header" class="card-header">
        <i class="el-icon-key card-header-icon"></i> Secret 列表
        <span class="card-count">{{ secrets.length }}</span>
      ...
      <el-alert
        ...
        :closable="false"
        class="secret-tip"
        title="Secret 是 Swarm 集群级资源，可供任意节点上的 Service Task 使用；内容出于安全考虑不可查看，仅展示元数据。" />
      ...
      <el-table
        ...
        <el-table-column label="操作" width="100" align="center">
          <template #default="{ row }">
            <el-tooltip content="该 Secret 仍被 Service 使用，无法删除" :disabled="!row.in_use" placement="left">
              <el-popconfirm
                title="确定删除该 Secret？此操作不可恢复"
                :disabled="row.in_use"
                @confirm="removeSecret(row)">
                <el-button
                  slot="reference"
                  type="text"
                  size="small"
...
<script>
import { clusterSwarmSecrets, clusterSwarmSecretDelete } from '@/api/cluster'
...
export default {
  ...
  computed: {
    ...
    filteredSecrets () {
      ...
      if (!kw) return this.secrets
      return this.secrets.filter(s => (s.name || '').toLowerCase().includes(kw))
    }
  },
  created () {
    this.load()
  ...
  methods: {
    load () {
      ...
      clusterSwarmSecrets(this.orgId, this.clusterId).then(res => {
        ...
        this.$message.error(err.response?.data?.msg || err.message || '加载 Secret 列表失败')
      }).finally(() => { this.loading = false })
    },
    openCreate () {
      this.$router.push({ name: 'ClusterSwarmSecretCreate', params: { clusterId: this.clusterId } })
    },
    removeSecret (row) {
      clusterSwarmSecretDelete(this.orgId, this.clusterId, row.id).then(() => {

<template>
  <div class="user-main box-shadow">
    ...
    <el-form
      ...
      <el-form-item label="LOGO" prop="fakelogo">
        <el-upload
          ref="avatar-uploader"
          class="avatar-uploader"
          :action="avatarUpload.action"
...
export default {
  ...
  data () {
    return {
      ...
      // 头像上传
      avatarUpload: {
        action: process.env.VUE_APP_BASE_API + 'org/logo/upload',
        headers: {
  ...
  methods: {
    ...
    submit () {
      this.$refs['main-form'].validate((valid) => {
        if (valid) {
          ...
          if (!file) {
            // 修复LOGO上传成功，但是创建失败之后无法再次提交表单的bug
            this.createOrg().then(res => {
              this.$message.success('创建成功')
              this.$store.dispatch('user/changeOrg', res.data.org).then(() => {
    ...
    handleAvatarChange (file, fileList) {
      if (file.status == 'ready') {
        ...
        this.mainForm.fakelogo = this.avatarUpload.previewSrc
      }
    },
    // 上传成功
